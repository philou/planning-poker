...
*** TODO send the new vote end timestamp along the message on start vote, use 30s for example
**** DONE rename the current broadcast
**** TODO remove the message param
**** TODO add an extra end vote date param
*** TODO change the current code to use a div instead of a direct <p>
*** TODO add some tests on the coffee side
*** TODO when receiving a message, replace the start vote button by the countdown, continue updating the sentence
*** TODO adapt the feature tests to check for the correct thing
**** TODO make it fail with bad code
**** TODO make it pass with good code
