...
** DONE display a real countdown in the client
...
** DONE is there a way to simplify constant declaration and import in coffeescript / javascript ? (team.coffee)
** TODO bundle update
*** DONE update the deps
*** DONE Fix deprecations
*** TODO fix the unique constraint
*** TODO find a way to spot this earlier ?
**** Create a new rake task that does db:drop, db:create, db:migrate on all envs and then only the default task
**** Does a similar task already exist ?
* TODO Finish retro
* Next Stories
* Smells
* Retrospective
...
| mad | sad                                                | glad                     |
|-----+----------------------------------------------------+--------------------------|
|     | not very fast ...                                  | finished this story !    |
|     | offline still not very effective                   | I have ideas for my book |
|     | Lot's of improvement to do                         |                          |
|     | I don't think I can finish for end of March        |                          |
|     | Not sure I'll remember much of what I learned here |                          |
*** I should never mix doing and learning !
*** TODO create 2 columns in my trello board : doing & learning
*** TODO only pure learning activities in learning : katas, moocs, exercices, 20h stuff, always isolated from complex context
